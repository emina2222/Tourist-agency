		...
		<div class="row">
			...
			<div class="col-md-6">
				<form:form class="well form-horizontal"
					action="${pageContext.request.contextPath }/sacuvajFormuAngazovanja" method="POST">
					<input type="hidden" name="idAranzmana" value="${idAranzmana}"/>
					<input type="hidden" name="grupa" value="${grupa}"/>
					
					<label>Izaberi vodiča:</label>
					<select class="form-control" name="vodicId" id="vodic">
					<c:forEach var="temp" items="${vodici}">
						<option id="${temp.getId()}" value="${temp.getId()}">${temp.getIme()} ${temp.getPrezime()}</option>
					</c:forEach>
					</select>
					...
					<c:choose>
						<c:when test="${grupa==2}">
							...
							<select class="form-control" name="datumId" id="datumi">
								<c:forEach var="temp" items="${ponude}">
									<option id="${temp.getId()}" value="${temp.getId()}">${temp.getDatumPocetka()}
										- ${temp.getDatumZavrsetka()}</option>
								</c:forEach>
							</select>
						</c:when>
						<c:when test="${grupa==0 || grupa==1}">
							<label>Izaberi datume:</label>
							<select class="form-control" name="datumId" id="datumi">
									<option id="${datumi.getIdAranzmana()}" value="${datumi.getIdAranzmana()}">${datumi.getDatumPolaska()}
										- ${datumi.getDatumDolaska()}</option>
							</select>
						</c:when>
					</c:choose>

					<p>
						Dnevnica *: <input name="dnevnica" class="form-control" type="number" required/>
					</p>
