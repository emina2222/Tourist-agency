			...
			<table class="table table-striped">
				<thead>
					<tr>
						...
						<th scope="col">Pansion</th>
						<th scope="col">Brisanje</th>
					</tr>
				</thead>
				<tbody>
				<c:forEach var="temp" items="${stavke}">
					<tr>
						<td>${temp.getNazivSmestaja()}</td>
						<td>${temp.getIdSmJce()}</td>
						<td>${temp.getIdPonude()}</td>
						<td>${temp.getCena()}</td>
						<td>${temp.getPansion()}</td>
						<td><a href="${pageContext.request.contextPath }/brisanje-stavke-${temp.getIdSmestaja()}-${temp.getIdSmJce()}-${temp.getIdPonude()}">Obriši</a></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
