		...
		<div class="row">
			...
			<div class="col-md-6">
				<form:form class="well form-horizontal"
					action="${pageContext.request.contextPath }/sacuvajFormuAtrakcije" method="POST">
					<input type="hidden" value="${idDestinacije}" name="idDestinacije"/>
					
					<label>Izaberi tip atrakcije:</label>
					<select class="form-control" name="tipId">
					<c:forEach var="temp" items="${tipovi}">
						<option id="${temp.getIdTipaAtrakcije()}" value="${temp.getIdTipaAtrakcije()}">${temp.getNaziv()}</option>
					</c:forEach>
					</select>
