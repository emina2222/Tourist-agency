		...
		<div class="row">
			...
			<div class="col-md-8 centar">
				<form:form class="well form-horizontal"
					action="${pageContext.request.contextPath }/processReservation"
					method="POST">
					<input type="hidden" name="idAranzmana" value="${idAranzmana}" />
					<input type="hidden" name="idSmestaja" value="${idSmestaja}" />
					<input type="hidden" name="idPonude" value="${idPonude}" />
					<input type="hidden" name="idPutnika" value="${putnik.idPutnika}" />
					<input type="hidden" name="brojKreveta" value="${brojKreveta}" />


					<p>Ime i prezime: ${putnik.ime} ${putnik.prezime}</p>
					<h3>Detalji o putovanju:</h3>
					<p class="stavke">${nazivAranzmana}</p>
					<p class="stavke">Smeštaj: ${nazivSmestaja}</p>
					<p class="stavke">Period: ${datumi}</p>
					<p class="stavke">Broj kreveta/putnika: ${brojKreveta}</p>
					<p class="stavke">Cena aranžmana: ${cena*brojKreveta} RSD</p>


					<p>Da li želite da doplatite putno osiguranje u iznosu od
						${cenaOsiguranja} RSD?</p>
					<input type="checkbox" name="osiguranjeCheck" /> Da
					<br>
					<br>
					<div class="centar">
