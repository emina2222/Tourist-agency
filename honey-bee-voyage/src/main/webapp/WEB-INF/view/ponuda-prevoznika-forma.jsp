		...
		<div class="row">
			...
			<div class="col-md-6">
				<form:form class="well form-horizontal"
					action="${pageContext.request.contextPath }/sacuvajFormuPonudePrevoznika" method="POST">
					<input type="hidden" name="idAranzmana" value="${idAranzmana}"/>
					
					<label>Izaberi prevoznika:</label>
					<select class="form-control" name="tipId" id="prevoznik">
					<c:forEach var="temp" items="${tipovi}">
						<option id="${temp.getIdPrevoznika()}" value="${temp.getIdPrevoznika()}">${temp.getNaziv()}</option>
					</c:forEach>
					</select>

						<label>Izaberi vozilo:</label> <select class="form-control"
							name="voziloId" id="vozilo">
						</select>
