	...
	<div class="row">
		...
		<div class="col-md-12">
			<table class="table table-striped">
				<thead>
					<tr>
						<th scope="col">Aranžman</th>
						<th scope="col">Vodič</th>
						<th scope="col">Dnevnica</th>
						<th scope="col">Datum od</th>
						<th scope="col">Datum do</th>
					</tr>
				</thead>
				<tbody>
				<c:forEach var="temp" items="${ang}">
					<tr>
						<td>${temp.getNazivAranzmana()}</td>
						<td>${temp.getImePrezime()}</td>
						<td>${temp.getDnevnica()}</td>
						<td><fmt:formatDate pattern="dd/MM/yyyy" value="${temp.getDatum_pocetka()}" /></td>
						<td><fmt:formatDate pattern="dd/MM/yyyy" value="${temp.getDatum_kraja()}" /></td>
						<td><a href="${pageContext.request.contextPath }/obrisi-angazovanje-${temp.getIdAranzmana()}-${temp.getIdVodica()}">Obriši</a></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
