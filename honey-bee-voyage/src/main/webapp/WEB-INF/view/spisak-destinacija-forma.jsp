		...
		<div class="row">
			...
			<div class="col-md-6">
				<form:form class="well form-horizontal"
					action="${pageContext.request.contextPath }/sacuvajFormuDest" method="POST">
					<input type="hidden" name="idAranzmana" value="${idAranzmana}"/>
					
					<p>
						Aranzman: <input type="text" class="form-control" value="${nazivAranzmana}"/>
					</p>
					
					<label>Izaberi destinaciju *:</label>
					<select class="form-control" name="destinacija" id="destinacija">
					<c:forEach var="temp" items="${destinacije}">
						<option id="${temp.getIdDestinacije()}" value="${temp.getIdDestinacije()}">${temp.getLokacija()}, ${temp.getDrzava()}</option>
					</c:forEach>
					</select>
